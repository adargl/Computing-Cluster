...
class NeutronLexer(QsciLexerCustom):
    ...
    def _init_theme(self, defaults):
        with open(self.theme, "r") as f:
            self.theme_json = json.load(f)

        if defaults is None:
            defaults: DefaultConfig = self.theme_json["theme"]["defaults"]

        # Default text settings
        self.setDefaultColor(QtGui.QColor(defaults["color"]))
        self.setDefaultPaper(QtGui.QColor(defaults["paper"]))
        self.setDefaultFont(QtGui.QFont(*defaults["font"]))
        ...
        for clr in colors:
            ...
            for k, v in clr[name].items():
                if k == "color":
                    self.setColor(QtGui.QColor(v), getattr(self, name.upper()))
                elif k == "paper-color":
                    self.setPaper(QtGui.QColor(v), getattr(self, name.upper()))
                elif k == "font":
                    try:
                        self.setFont(
                            QtGui.QFont(
                                ...
                                v.get("font-size", 14),
                                self.font_weights.get(v.get("font-weight", QtGui.QFont.Weight.Normal)),
                                v.get("italic", False)
                            ),
